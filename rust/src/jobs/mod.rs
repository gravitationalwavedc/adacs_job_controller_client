use crate::messaging::{Message, Priority, UPDATE_JOB, SYSTEM_SOURCE, SUBMITTED, ERROR, COMPLETED, CANCELLED, DELETED, RUNNING};
...
lazy_static! {
    static ref SUBMIT_MUTEX: TokioMutex<()> = TokioMutex::new(());
}
...
pub async fn handle_job_submit(mut msg: Message) {
    ...
    tokio::spawn(async move {
        ...
        let mut details = get_default_job_details();
        {
            ...
            job_model = match db::get_or_create_by_job_id(db, job_id).await {
                Ok(j) => j,
                Err(e) => {
            ...
            if job_model.submitting {
                ...
                if job_model.submitting_count >= MAX_SUBMIT_COUNT {
                    ...
                    let _ = db::save_job(db, job_model).await;
                    return;
                }
            }

            if job_model.job_id.unwrap_or(0) != 0 && job_model.job_id.unwrap_or(0) == job_id {
                info!("Job with ID {} has already been submitted, checking status...", job_id);
            ...
            job_model.working_directory = "".to_string();
            job_model = db::save_job(db, job_model).await.expect("Failed to save job");
        }

        let bm = BundleManager::singleton();
        let working_dir = unsafe {
            bm.run_bundle_string("working_directory", &bundle_hash, &details, "")
        };
        job_model.working_directory = working_dir;
        job_model = db::save_job(db, job_model).await.expect("Failed to save job");

        let scheduler_id = unsafe {
            bm.run_bundle_uint64("submit", &bundle_hash, &details, &params)
        };
        ...
        if scheduler_id == 0 {
            ...
            let _ = db::delete_job(db, job_model.id).await;
            ...
            ws.queue_message(job_id.to_string(), result.get_data().clone(), Priority::Medium, Arc::new(|| {}));
            ...
            info!("Successfully submitted job with UI ID {}, got scheduler id {}", job_id, scheduler_id);
...
pub async fn check_job_status(job: job::Model, force_notification: bool) {
    ...
    let status_json = unsafe {
        bm.run_bundle_json("status", &job.bundle_hash, &details, "")
    };
    info!("A. Status Done");
    ...
    if let Some(statuses) = status_json["status"].as_array() {
        for stat in statuses {
            ...
            let mut v_status = db::get_job_status_by_job_id_and_what(db, job.id, what).await.unwrap_or_default();

            if v_status.len() > 1 {
                let ids: Vec<i64> = v_status.iter().map(|s| s.id).collect();
                let _ = db::delete_status_by_id_list(db, ids).await;
                v_status = vec![];
            }
            ...
            if force_notification || v_status.is_empty() || status != v_status[0].state as u32 {
                ...
                let _ = db::save_status(db, state_item).await;

                let ws = get_websocket_client();
                let mut result = Message::new(UPDATE_JOB, Priority::Medium, &job.job_id.unwrap_or(0).to_string());
                result.push_uint(job.job_id.unwrap_or(0) as u32);
                result.push_string(what);
                result.push_uint(status);
                result.push_string(info);
                ws.queue_message(job.job_id.unwrap_or(0).to_string(), result.get_data().clone(), Priority::Medium, Arc::new(|| {}));
                info!("A. update message on ws done");
            }
        }
    }

    let v_status = db::get_job_status_by_job_id(db, job.id).await.unwrap_or_default();
    ...
    if job_error != 0 || (status_json["complete"].as_bool().unwrap_or(false) && job_complete) {
        ...
        let _ = db::save_job(db, job_to_save).await;
...
pub async fn check_all_jobs_status() {
    ...
    let jobs = match db::get_running_jobs(db).await {
        Ok(j) => j,
        Err(e) => {
...
pub async fn archive_job(job: &job::Model) -> bool {
    ...
    info!("Archiving job {} completed with code {}\n\nStdout: {}\n\nStderr: {}", 
...
pub async fn handle_job_cancel(mut msg: Message) {
    tokio::spawn(async move {
        let db = db::get_db();
        let job_id = msg.pop_uint() as i64;

        let mut job_model = match db::get_or_create_by_job_id(db, job_id).await {
            Ok(j) => j,
            Err(_) => return,
        };

        if job_model.id == 0 || !job_model.running || job_model.submitting {
            error!("Job does not exist ({}), or job is in an invalid state", job_id);
        ...
        job_model = db::get_job_by_id(db, job_model.id).await.unwrap().unwrap();

        if !job_model.running {
            warn!("Job {} is not running so cannot be cancelled, nothing to do.", job_id);
            return;
        }
        ...
        let cancelled = unsafe {
            bm.run_bundle_bool("cancel", &job_model.bundle_hash, &details, "")
        };
        ...
        let db_status = db::get_job_status_by_job_id(db, job_model.id).await.unwrap_or_default();
        if db_status.iter().any(|s| s.state as u32 == CANCELLED) {
            return;
        }

        job_model.running = false;
        let _ = db::save_job(db, job_model.clone()).await;
...
pub async fn handle_job_delete(mut msg: Message) {
    tokio::spawn(async move {
        let db = db::get_db();
        let job_id = msg.pop_uint() as i64;
        ...
        if job_model.id == 0 || job_model.running || job_model.submitting || job_model.deleted {
            error!("Job does not exist ({}), is currently running, or has already been deleted.", job_id);
        ...
        let bm = BundleManager::singleton();
        let deleted = unsafe {
            bm.run_bundle_bool("delete", &job_model.bundle_hash, &details, "")
        };

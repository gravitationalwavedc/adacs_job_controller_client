...
use crate::messaging::{Message, Priority,
    DB_BUNDLE_CREATE_OR_UPDATE_JOB, DB_BUNDLE_GET_JOB_BY_ID, DB_BUNDLE_DELETE_JOB};
...
unsafe extern "C" fn create_or_update_job(_self: *mut PyObject, args: *mut PyObject) -> *mut PyObject {
    ...
    let job_data: serde_json::Value = serde_json::from_str(&json_str).unwrap_or(serde_json::Value::Null);
    ...
    log::info!("DB: create_or_update_job req - bundle hash: {}, jobId: {}", bundle_hash, job_id);
    ...
    match send_and_wait(msg) {
        Ok(response) => {
            ...
            log::info!("DB: create_or_update_job res - jobId: {}", new_job_id);
...
unsafe extern "C" fn get_job_by_id(_self: *mut PyObject, args: *mut PyObject) -> *mut PyObject {
    ...
    log::info!("DB: get_job_by_id req - bundle hash: {}, jobId: {}", bundle_hash, job_id);
    ...
    match send_and_wait(msg) {
        Ok(response) => {
            ...
            if !success {
                let err_msg = CString::new(format!("Job with ID {} does not exist.", job_id)).unwrap();
                PyErr_SetString(BUNDLE_DB_ERROR, err_msg.as_ptr());
                return ptr::null_mut();
            }
            ...
            log::info!("DB: get_job_by_id res - data: {}", job_data_json);
...
unsafe extern "C" fn delete_job(_self: *mut PyObject, args: *mut PyObject) -> *mut PyObject {
    ...
    if job_id == 0 {
        ...
        unsafe { PyErr_SetString(BUNDLE_DB_ERROR, err_msg.as_ptr()); }
        return ptr::null_mut();
    }
    ...
    log::info!("DB: delete_job req - bundle hash: {}, jobId: {}", bundle_hash, job_id);
    ...
    match send_and_wait(msg) {
        Ok(response) => {
            ...
            log::info!("DB: delete_job res - success");
...
pub unsafe extern "C" fn PyInit_bundledb() -> *mut PyObject {
    unsafe {
        (*(&raw mut BUNDLE_DB_MODULE)).m_methods = (&raw mut BUNDLE_DB_METHODS) as *mut PyMethodDef;

...
impl BundleFixture {
    ...
    fn write_script(&self, hash: &str, script: &str, replacements: &[(&str, &str)]) {
        ...
        fs::write(bundle_path.join("bundle.py"), script_content).expect("Failed to write bundle.py");
    }
    ...
    pub fn write_job_submit(&self, hash: &str, working_dir: &str, scheduler_id: &str) {
        self.write_script(hash, JOB_SUBMIT_SCRIPT, &[
    ...
    pub fn write_job_submit_error(&self, hash: &str, error_return: &str) {
        self.write_script(hash, JOB_SUBMIT_ERROR_SCRIPT, &[
...
"#, return_val);
    ...
    pub fn write_job_submit_check_status(&self, hash: &str, working_dir: &str, scheduler_id: &str, status_json: &str) {
        self.write_script(hash, JOB_STATUS_SCRIPT, &[
            ("WORKING_DIR", working_dir),
            ("SCHEDULER_ID", scheduler_id),

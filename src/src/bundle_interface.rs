...
// function that sets a Python error always returns NULL, so the existing
// bundle-raising test only covers the `p_result.is_null()` side. This seam
// lets tests force `PyObject_CallObject` to return a non-NULL result while
// setting an error without changing production behavior. Tests run serially

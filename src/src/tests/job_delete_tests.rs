...
#[test_fork::test]
fn test_delete_job_save_as_deleted_failure_logs_error() {
    let db_name = Uuid::new_v4().to_string();
    let (fixture, bundle_hash, state, _working_dir) = setup_delete_test(&db_name);
    ...
    // Custom mock: the first DB_JOB_SAVE (for delete) succeeds, while the
    // final DB_JOB_SAVE (as deleted) returns saved_id = 0 to trigger the
    // error-logging branch in handle_job_delete.
    ...
    assert!(
        logs.contains("Failed to save job 1234 as deleted"),
        "expected as-deleted save failure in logs, got:\n{logs}"
    );
}

...
#[test_fork::test]
fn test_get_file_upload_connect_failure() {
    #[tokio::main(flavor = "current_thread")]
    async fn inner() {
        setup_test("test_ul_connect_failure");

        let mut mock_ws = MockWebsocketClient::new();
        let (tx, mut rx) = tokio::sync::mpsc::unbounded_channel();
        let tx_clone = tx.clone();

        let test_uuid = "test-uuid-ul-connect-failure".to_string();
        let uuid_clone = test_uuid.clone();
        mock_ws
            .expect_queue_message()
            .with(eq(uuid_clone), always(), eq(Priority::Highest))
        ...
        let response = tokio::time::timeout(Duration::from_secs(2), rx.recv())
            .await
            .expect("Timeout waiting for FILE_UPLOAD_ERROR")
            .expect("No response");
...
#[test_fork::test]
fn test_get_file_download_job_file_open_failed() {
    #[tokio::main(flavor = "current_thread")]
    async fn inner() {
        setup_test("test_dl_open_failed");

        let fixture = TemporaryDirectoryFixture::new();
        let working_dir = fixture.get_temp_path().to_str().unwrap().to_string();
        let file_path = fixture.get_temp_path().join("unreadable_download.txt");
        fs::write(&file_path, b"content").unwrap();
        // A mode-000 regular file lets fs::metadata succeed (stat works) while
        // File::open fails with EACCES when tests run as non-root.
        #[cfg(unix)]
        {
            use std::os::unix::fs::PermissionsExt;
        ...
        let test_uuid = "test-uuid-dl-open-failed".to_string();
        let mut msg_raw = Message::new(FILE_DOWNLOAD, Priority::Highest, SYSTEM_SOURCE);
        msg_raw.push_uint(job_id as u32);
        msg_raw.push_string(&test_uuid);
        msg_raw.push_string("some_hash");
        msg_raw.push_string("unreadable_download.txt");
        ...
        let mut server = server;
        // FILE_DOWNLOAD_DETAILS is sent before File::open, so consume it first.
...
#[test_fork::test]
fn test_file_upload_ignores_stray_binary_frame() {
    #[tokio::main(flavor = "current_thread")]
    async fn inner() {
        setup_test("test_file_upload_ignores_stray_binary_frame");
        ...
        let test_uuid = "test-uuid-upload-stray-binary".to_string();
        let file_content = b"uploaded content";
        let target_path = "subdir/uploaded.txt";
        ...
        // A stray binary frame with an unrelated message id (not FILE_UPLOAD_CHUNK
        // nor FILE_UPLOAD_COMPLETE) must be ignored and the upload must continue.
        let stray_msg = Message::new(FILE_DOWNLOAD, Priority::Highest, SYSTEM_SOURCE);
        server.msg_tx.send(stray_msg.get_data().clone()).unwrap();
...
#[test_fork::test]
fn test_file_upload_partial_file_cleanup_on_peer_close() {
    #[tokio::main(flavor = "current_thread")]
    async fn inner() {
        setup_test("test37");
        ...
        let test_uuid = "test-uuid-upload-peer-close".to_string();
        let target_path = "partial_file.txt";
        let declared_size = 1000u64;
        let actual_size = 500u64; // Send less than declared
        ...
        // Wait for the client to create the partial file before sending the
        // peer Close so the cleanup path is genuinely exercised.
        let full_path = fixture.get_temp_path().join(target_path);
        let created_deadline = std::time::Instant::now() + Duration::from_secs(2);
        while !full_path.exists() {
            assert!(
        ...
        // Peer sends a genuine WebSocket Close frame before FILE_UPLOAD_COMPLETE
        ...
        while full_path.exists() {
            assert!(
                std::time::Instant::now() < cleaned_deadline,
                "Partial file should have been cleaned up after peer Close"
            );
            tokio::time::sleep(Duration::from_millis(10)).await;
        }
    } // end inner()
...
#[test_fork::test]
fn test_file_upload_mid_transfer_ping_is_ignored_and_complete_commits() {
    #[tokio::main(flavor = "current_thread")]
    async fn inner() {
        setup_test("test_upload_ping");
        ...
        let test_uuid = "test-uuid-upload-ping".to_string();
        ...
        // Send the first chunk, then inject a WebSocket Ping control frame
        // mid-transfer. The upload receive loop must ignore the Ping (it is
        // neither a chunk nor a Close) and continue to FILE_UPLOAD_COMPLETE.
        let mut first_chunk = Message::new(FILE_UPLOAD_CHUNK, Priority::Highest, &test_uuid);
        first_chunk.push_bytes(&file_content[..64 * 1024]);
        server.msg_tx.send(first_chunk.get_data().clone()).unwrap();

        server.send_peer_ping().await;
        tokio::time::sleep(Duration::from_millis(50)).await;

        let mut second_chunk = Message::new(FILE_UPLOAD_CHUNK, Priority::Highest, &test_uuid);
        second_chunk.push_bytes(&file_content[64 * 1024..]);
        server.msg_tx.send(second_chunk.get_data().clone()).unwrap();
        ...
        let response = tokio::time::timeout(Duration::from_secs(5), server.msg_rx.recv())
            ...
            .expect("No response");
        assert_eq!(
            response.id, FILE_UPLOAD_COMPLETE,
            "mid-transfer Ping must be ignored and the upload must still commit FILE_UPLOAD_COMPLETE"
        );

        let final_path = Path::new(&working_dir).join(target_path);
        assert_eq!(fs::metadata(final_path).unwrap().len(), 128 * 1024);
    } // end inner()
    inner();
}
...
#[test_fork::test]
fn test_file_upload_zero_declared_with_chunk() {
    #[tokio::main(flavor = "current_thread")]
    async fn inner() {
        setup_test("test18");
        ...
        msg_raw.push_string("some_hash");
        msg_raw.push_string(target_path);
        msg_raw.push_ulong(0); // Declare 0 bytes
        ...
        // A non-empty chunk against a zero-byte file must be rejected
        // immediately without waiting for FILE_UPLOAD_COMPLETE.
        let mut chunk_msg = Message::new(FILE_UPLOAD_CHUNK, Priority::Highest, &test_uuid);
        chunk_msg.push_bytes(&[0u8; 20]);
        server.msg_tx.send(chunk_msg.get_data().clone()).unwrap();
...
/// A mid-transfer chunk-send failure must select the `"chunk send failed"`
/// primary error. We park the supervisor just before a chunk send via the
/// pre-chunk-send barrier, reset the fixture peer transport (`SO_LINGER=0`
/// RST) so the chunk send deterministically fails, release the barrier, and
/// assert the authoritative result is `PrimaryError("chunk send failed")`
/// and the connection is released.
#[test_fork::test]
fn test_task4_chunk_send_failure_selects_primary_error() {
    #[tokio::main(flavor = "current_thread")]
    async fn inner() {
        setup_test("task4_chunk_send_failure");
        ...
        // Park the supervisor before the first chunk send, then reset the peer
        // transport so the chunk send deterministically fails.
        tokio::time::timeout(Duration::from_secs(2), barrier.wait_until_reached())
            .await
        ...
        // The authoritative result must be the chunk-send primary error.
        let outcome = tokio::time::timeout(Duration::from_secs(2), outcome_rx.recv())
            .await
            .expect("supervisor must report an authoritative result")
            .expect("outcome channel closed");
        assert!(
            matches!(outcome, TransferOutcome::PrimaryError(ref m) if m == "chunk send failed"),
            "chunk-send failure must select the primary error, got {outcome:?}"
        );

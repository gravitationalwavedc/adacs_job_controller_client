...
// The `PyDict_New` failure branches in `BundleInterface::new` (the globals
// and locals dicts) are unreachable through the public API because
// `PyDict_New` always succeeds in practice. This seam lets tests force it to
// return NULL without changing production behavior. Tests run serially

...
#[cfg(test)]
static PY_OBJECT_CALLOBJECT_OVERRIDE: Mutex<Option<PyObjectCallObjectFn>> = Mutex::new(None);

/// Test-only: install an override for `py_object_callobject`, returning the
/// previously-installed override (if any). Pass `None` to clear it.
#[cfg(test)]
pub fn set_py_object_callobject_override(
    f: Option<PyObjectCallObjectFn>,
) -> Option<PyObjectCallObjectFn> {
    let mut guard = PY_OBJECT_CALLOBJECT_OVERRIDE.lock();
    std::mem::replace(&mut *guard, f)
}
...
/// the GIL; `callable` is a valid callable, `args` is a valid tuple.
pub unsafe fn py_object_callobject(callable: *mut PyObject, args: *mut PyObject) -> *mut PyObject {
    #[cfg(test)]
    if let Some(f) = *PY_OBJECT_CALLOBJECT_OVERRIDE.lock() {

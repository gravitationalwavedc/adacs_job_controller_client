...
/// NULL or valid references.
#[cfg(test)]
pub type PyErrNewExceptionFn =
    unsafe fn(*const c_char, *mut PyObject, *mut PyObject) -> *mut PyObject;

#[cfg(test)]
static PY_ERR_NEWEXCEPTION_OVERRIDE: Mutex<Option<PyErrNewExceptionFn>> = Mutex::new(None);

/// Test-only: install an override for `py_err_newexception`, returning the
/// previously-installed override (if any). Pass `None` to clear it.
#[cfg(test)]
pub fn set_py_err_newexception_override(
    f: Option<PyErrNewExceptionFn>,
) -> Option<PyErrNewExceptionFn> {
    let mut guard = PY_ERR_NEWEXCEPTION_OVERRIDE.lock();
    std::mem::replace(&mut *guard, f)
}

pub unsafe fn py_err_newexception(
    name: *const c_char,
    base: *mut PyObject,
    dict: *mut PyObject,
) -> *mut PyObject {
    #[cfg(test)]
    if let Some(f) = *PY_ERR_NEWEXCEPTION_OVERRIDE.lock() {

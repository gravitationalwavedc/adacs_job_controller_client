...
/// Test-only: install an override for `py_module_addobject`, returning the
/// previously-installed override (if any). Pass `None` to clear it.
#[cfg(test)]
pub fn set_py_module_addobject_override(
...
/// the GIL; `module` is a valid module, `name` is a NUL-terminated string,
/// `value` is a live object (ownership is transferred on success).
pub unsafe fn py_module_addobject(
    module: *mut PyObject,
    name: *const c_char,
    value: *mut PyObject,
) -> c_int {

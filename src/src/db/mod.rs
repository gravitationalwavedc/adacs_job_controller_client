...
mod tests {
    use super::*;
    use crate::messaging::{DB_JOBSTATUS_GET_BY_JOB_ID, DB_RESPONSE};
    use crate::websocket::{
    ...
    #[test]
    fn get_job_status_by_job_id_parses_multiple_statuses() {
        let _guard = TEST_MUTEX.lock().unwrap();
        reset_websocket_client_for_test();
        let mut mock = MockWebsocketClient::new();
        mock.expect_send_db_request().times(1).returning(|message| {
            ...
            resp.push_ulong(1);
            resp.push_ulong(42);
            resp.push_string("slurm");
            resp.push_uint(500);
            resp.push_ulong(2);
            resp.push_ulong(42);
            resp.push_string("pbs");
            resp.push_uint(30);
            Box::pin(async move { Ok(resp) })
        });
        set_websocket_client(Arc::new(mock));
        ...
        assert_eq!(statuses[0].what, "slurm");
        assert_eq!(statuses[0].state, 500);
        assert_eq!(statuses[1].id, 2);
        assert_eq!(statuses[1].what, "pbs");
        assert_eq!(statuses[1].state, 30);
    }
    ...
    fn get_job_status_by_job_id_returns_empty_when_count_zero() {
        ...
        let rt = tokio::runtime::Runtime::new().unwrap();
        let statuses = rt
            .block_on(async { get_job_status_by_job_id(99).await })
            .unwrap();

...
mod tests {
    ...
    #[test]
    fn get_job_by_job_id_returns_error_on_websocket_failure() {
        let _guard = TEST_MUTEX.lock().unwrap();
        reset_websocket_client_for_test();
        let mut mock = MockWebsocketClient::new();
        mock.expect_send_db_request()
        ...
        let rt = tokio::runtime::Runtime::new().unwrap();
        let result = rt.block_on(async { get_job_by_job_id(99).await });

        assert!(result.is_err());
        assert!(
            result.unwrap_err().contains("websocket send failed"),
            "error should propagate websocket failure"
        );
    }

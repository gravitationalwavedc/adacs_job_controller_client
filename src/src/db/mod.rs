...
mod tests {
    ...
    #[test]
    fn get_job_status_by_job_id_and_what_returns_error_when_websocket_send_fails() {
        let _guard = TEST_MUTEX.lock().unwrap();
        reset_websocket_client_for_test();
        let mut mock = MockWebsocketClient::new();
        mock.expect_send_db_request().times(1).returning(|_| {
            Box::pin(async move {
                Err::<Message, Box<dyn std::error::Error + Send + Sync>>("connection reset".into())
            })
        });
        set_websocket_client(Arc::new(mock));
        ...
        let rt = tokio::runtime::Runtime::new().unwrap();
        let result =
            rt.block_on(async { get_job_status_by_job_id_and_what(42, "scheduler_id").await });

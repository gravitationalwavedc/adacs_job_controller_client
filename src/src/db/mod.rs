...
mod tests {
    ...
    fn delete_status_by_id_list_sends_count_then_ids() {
        ...
        mock.expect_send_db_request().times(1).returning(|message| {
            ...
            assert_eq!(parsed.pop_ulong(), 20);
            assert_eq!(parsed.pop_ulong(), 30);
        ...
        let rt = tokio::runtime::Runtime::new().unwrap();
        rt.block_on(async { delete_status_by_id_list(vec![10, 20, 30]).await })
            .unwrap();
    }

    #[test]
    fn delete_status_by_id_list_sends_zero_count_for_empty_list() {
        let _guard = TEST_MUTEX.lock().unwrap();
        reset_websocket_client_for_test();
        let mut mock = MockWebsocketClient::new();
        mock.expect_send_db_request().times(1).returning(|message| {
        ...
        let rt = tokio::runtime::Runtime::new().unwrap();
        rt.block_on(async { delete_status_by_id_list(vec![]).await })
            .unwrap();
    }

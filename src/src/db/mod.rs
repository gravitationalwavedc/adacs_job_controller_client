...
mod tests {
    use super::*;
    use crate::messaging::{DB_JOB_GET_BY_ID, DB_RESPONSE};
    use crate::websocket::{
    ...
    #[test]
    fn get_job_by_id_returns_none_when_server_sends_zero_count() {
        let _guard = TEST_MUTEX.lock().unwrap();
        reset_websocket_client_for_test();
        let mut mock = MockWebsocketClient::new();
        ...
        let rt = tokio::runtime::Runtime::new().unwrap();
        let result = rt.block_on(async { get_job_by_id(99).await }).unwrap();

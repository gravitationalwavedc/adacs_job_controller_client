...
mod tests {
    ...
    #[test]
    fn get_or_create_by_job_id_returns_existing_job() {
        let _guard = TEST_MUTEX.lock().unwrap();
        reset_websocket_client_for_test();
        let mut mock = MockWebsocketClient::new();
        mock.expect_send_db_request().times(1).returning(|_| {
    ...
    #[test]
    fn get_or_create_by_job_id_returns_default_when_not_found() {
        let _guard = TEST_MUTEX.lock().unwrap();
        reset_websocket_client_for_test();
        let mut mock = MockWebsocketClient::new();
        mock.expect_send_db_request().times(1).returning(|_| {
        ...
        let rt = tokio::runtime::Runtime::new().unwrap();
        let job = rt
            .block_on(async { get_or_create_by_job_id(999).await })
            .unwrap();

        assert_eq!(job.id, 0);
        assert_eq!(job.job_id, None);
        assert_eq!(job.scheduler_id, None);
        assert!(!job.submitting);
        assert_eq!(job.submitting_count, 0);
        assert!(job.bundle_hash.is_empty());
        assert!(job.working_directory.is_empty());
        assert!(!job.running);
        assert!(!job.deleted);
        assert!(!job.deleting);
    }
